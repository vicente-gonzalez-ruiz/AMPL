...
var t{Products, Months}; # Amount of product i in month j to be stored
var a{Products, Months} binary; # 1 if product i in month j is produced

maximize profit: sum{i in Products, j in Months} (unit_price[i]*y[i, j] - production_cost[i]*x[i, j] - storage_rage[i]*t[i, j] - activation_cost[i]*a[i, j]);
...
subjecto to max_sold{i in Products, j in Months}: y[i, j] <= demand[i, j];

subject to minimum_production{i in Products, j in Months}: minimum_batch[i]*a[i, j] <= x[i, j];

subject to maximum_production{i in Products, j in Months}: x[i, j]/(maximum_quota[i] * productive_days[j] <= a[i, j];

subject to storage_remain{i in Products, jo in Months}: t[i, j-1]+x[i, j] = y[i, j] + t[i, j];
